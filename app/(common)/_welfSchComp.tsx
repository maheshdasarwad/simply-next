...
const SidebarHeader = ({
  ...
  <div className={`relative flex items-center w-full ${isMobile ? 'h-16 mb-2' : 'h-20 mt-4 mb-3'}`}>
    <div className={`flex flex-col items-center w-full ${isMobile ? 'mb-2' : 'mb-4'}`}>
      <img
        src="/Images/logo2.png"
        alt="Simply Saral Logo"
        className={`object-contain transition-all duration-300 ${
          open ? "w-16 h-16 md:w-20 md:h-20" : "w-12 h-12 md:w-16 md:h-16"
        }`}
      />
      {open && (
        <div
    ...
    {!isMobile && (
      <button
        ...
        className="absolute -right-3 top-1/2 -translate-y-1/2 z-10 border-2 border-blue-400 rounded-full w-6 h-6 md:w-8 md:h-8 flex items-center justify-center shadow-xl transition-transform duration-300 group bg-white dark:bg-gray-900"
      >
        {open
          ? React.createElement(iconMap["ChevronLeft"], { className: "text-blue-500 w-4 h-4 md:w-5 md:h-5" })
          : React.createElement(iconMap["ChevronRight"], { className: "text-blue-500 w-4 h-4 md:w-5 md:h-5" })}
      </button>
    )}
  </div>
);
...
// Safe icon component to handle invalid icon names
const SafeIcon = ({ 
  name, 
  className = "",
  fallback = "Sparkles" 
...
const MobileSidebarTrigger = ({
  ...
  return (
    <button
      ...
      className={`lg:hidden fixed top-4 left-4 z-50 p-3 rounded-full shadow-lg transition-all duration-300 ${
        isDark 
...
const Sidebar = ({
  ...
  const isDark = theme === "dark";
  const isMobile = typeof mobileOpen !== 'undefined';
  ...
  const renderNavigationItem = (item: typeof NAVIGATION_ITEMS[0]) => (
    <li key={item.label}>
      <button
        ...
        className={`w-full flex ${open ? "flex-row items-center gap-3 px-3" : "flex-col items-center gap-1 px-2"} py-3 rounded-lg transition-colors ${
      ...
      >
        <SafeIcon 
          name={item.icon} 
          className={`flex-shrink-0 transition-all duration-300 ${open ? "w-5 h-5" : "w-6 h-6"}`} 
        />
  ...
  const renderFilterSection = (section: FilterCategory) => (
    <div key={section.heading} className="mb-2">
      <button
        ...
        className={`w-full flex items-center justify-between px-3 py-3 rounded-lg font-semibold uppercase tracking-wide text-xs ${
  ...
  const sidebarContent = (
    <>
        ...
        {isMobile && (
          <button
            ...
            className={`absolute top-4 right-4 p-2 rounded-lg ${
              isDark ? "hover:bg-gray-800 text-white" : "hover:bg-blue-50 text-blue-700"
            }`}
          >
            <X className="w-5 h-5" />
          </button>
      ...
      <div className={`flex-shrink-0 border-t ${isDark ? "border-gray-700" : "border-blue-400"} p-2 space-y-2`}>
        ...
        >
          {isDark ? (
            <SafeIcon name="Sun" className={`flex-shrink-0 transition-all duration-300 ${open ? "w-5 h-5" : "w-6 h-6"}`} />
          ) : (
            <SafeIcon name="Moon" className={`flex-shrink-0 transition-all duration-300 ${open ? "w-5 h-5" : "w-6 h-6"}`} />
          )}
        ...
        <button
          ...
          title="Profile"
        >
          <SafeIcon name="UserCircle" className={`flex-shrink-0 transition-all duration-300 ${open ? "w-5 h-5" : "w-6 h-6"}`} />
...
const SchemeCard = ({ scheme, theme, accentColor }: { scheme: SchemeData; theme: "light" | "dark"; accentColor?: { light: string; dark: string } }) => {
  ...
  return (
    <div className={`rounded-xl p-4 md:p-6 transition-all duration-400 hover:shadow-xl border ${
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className={`text-lg md:text-xl font-semibold ${isDark ? `text-${darkAccent}` : `text-${lightAccent}`}`}>
              {scheme.title}
            </h3>
          </div>
          <div className={`text-sm flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 ${isDark ? "text-gray-400" : "text-gray-600"}`}>
        ...
        <div className="flex items-center justify-center">
          <a
            ...
            className={`inline-flex items-center px-4 py-2 rounded-lg font-medium text-sm md:text-base transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 relative overflow-hidden group ${
    ...
    >
      ...
      <div className={`transition-all duration-300 ${
        ...
        <div className="p-4 md:p-6">
          ...
          <div className="bg-black rounded-xl md:rounded-2xl overflow-hidden mb-4 md:mb-6 h-48 md:h-64 lg:h-80 relative shadow-2xl">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent px-4 md:px-8 py-4 md:py-6 lg:py-8">
              <h2 className="text-lg md:text-2xl lg:text-3xl font-bold mb-2 tracking-wide text-white drop-shadow-2xl line-clamp-2">
                {carouselSlides[currentSlide].title}
              </h2>
              <p className="text-sm md:text-lg lg:text-xl text-gray-100 font-medium drop-shadow-lg max-w-3xl line-clamp-2 md:line-clamp-3">
                {carouselSlides[currentSlide].subtitle}
              </p>
            </div>
            <div className="absolute bottom-4 md:bottom-6 right-4 md:right-8 flex gap-2">
          ...
          <div className={`rounded-xl md:rounded-2xl p-4 md:p-6 mb-4 md:mb-6 shadow-xl ${
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-4">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-3 py-2.5 md:py-3 rounded-lg md:rounded-xl border text-sm md:text-base transition-all duration-300 ${
                    theme === "dark"
              ...
              <button
                ...
                className={`flex items-center justify-center gap-2 px-4 py-2.5 md:px-6 md:py-3 rounded-lg md:rounded-xl font-semibold border transition-all duration-300 ${
                  showFilters
                    ? theme === "dark"
                  ...
                  <div className="flex items-center justify-between mb-3 md:mb-4">
                    ...
                    {selectedCategories.length > 0 && (
                      <button
                        ...
                        className={`text-xs md:text-sm px-2 py-1 md:px-3 md:py-1 rounded-lg ${
                          theme === "dark" 
                  ...
                  <div className="flex flex-wrap gap-2 md:gap-3">
                    {categoryOptions.map((option) => (
                      <button
                        ...
                        className={`px-3 py-1.5 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium border-2 transition-all duration-300 flex items-center gap-1 md:gap-2 ${
                      ...
                      >
                        <div className={`w-3 h-3 md:w-4 md:h-4 border rounded flex items-center justify-center ${
                          ...
                          {selectedCategories.includes(option) && (
                            <div className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-sm ${
                              theme === "dark" ? "bg-orange-600" : "bg-blue-600"
                            }`} />
                          )}
                        </div>
                  ...
                  <div className="flex items-center justify-between mb-3 md:mb-4">
                    <div className="flex items-center gap-2 md:gap-3">
                      ...
                      <span className={`font-bold text-sm md:text-base ${
                        ...
                        Filter by Launch Year {selectedYears.length > 0 && `(${selectedYears.length} selected)`}
                      </span>
                    </div>
                  ...
                  <p className={`text-xs md:text-sm ${
                    ...
                    {selectedCategory !== "All" && (
                      <span className="ml-1 md:ml-2">
                        • Sidebar filter: <span className="font-semibold text-blue-600">{selectedCategory}</span>
                      </span>
                    )}
                  </p>
                  <button
                    onClick={clearAllFilters}
                    className={`text-xs md:text-sm px-3 py-1.5 rounded-lg self-start sm:self-auto ${
                      theme === "dark" 
                        ? "text-orange-400 hover:bg-orange-400/20" 
          ...
          {displayedSchemes.length > 0 ? (
            ...
            <div className={`text-center py-12 md:py-16 rounded-xl ${
              theme === "dark" ? "bg-slate-900" : "bg-white"
            } shadow-lg`}>
              <SafeIcon
                name="Search"
              ...
              {hasActiveFilters && (
                <button
                  onClick={clearAllFilters}
                  className={`mt-3 md:mt-4 px-4 py-2 md:px-6 md:py-2 rounded-lg font-medium text-sm md:text-base ${
                    theme === "dark" 
        ...
        {showBackToTop && (
          <button
            onClick={scrollToTop}
            className={`fixed bottom-6 right-6 md:bottom-8 md:right-8 w-10 h-10 md:w-12 md:h-12 rounded-full shadow-xl flex items-center justify-center transition-all duration-300 hover:scale-110 group ${
